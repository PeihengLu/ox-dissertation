...
However, the transformer model is not without its drawbacks. The self-attention mechanism is very computationally expensive, with a time complexity of $O(n^2)$, where n is the length of the input sequence. This makes the transformer model unsuitable for long sequences, as the memory and time requirements grow quadratically with the sequence length.
...
The hyperparameter tuning of the model was performed on the PRIDICT2.0 HEK293T for a balance between dataset size and training time. This also has the added benefit of reducing the bias of the model on a particular fold of the dataset, but it still would not eliminate the bias of the hyperparameters on the HEK293T and PE2 cellline-editor combination.

Similar to PRIDICT, the size of the MLP layers were controled using three parameters. The first is the mlp\_embed\_factor, which controls the size of the hidden layers in the MLP encoder and decoder. This is followed by the embed dimension of the MLP, which is the size of the output of the MLP embedder processing the feature vector. The last parameter is the number of encoder units inside of the MLP, where a encoder unit is a combination of two linear layers converting the data from embed dimension to embed dimention * mlp\_embed\_factor, followed by dropout and layer normalization.

To reduce the computational cost by testing many parameters at the same times, the tuning was conducted in three stages.

The first stage of tuning is used to determine the optimal training configuration, in particular the loss function, learning rate and batch sizes of the model. 

The second stage of tuning focuses on the a number of design decisions of the model architecture. Since the number of parameters between the designs does not differ significantly, a small network with two hidden layers of size 2*embedding dimension and two attention heads was trained on a subset (20\%) of the dataset to further speedup the tuning process.

% To begin with, the actual embedding of the nucleotide dataset was tested. Although both yu et al and mathis et al reported the usage of one-hot encoding in their publication, the actual implementation were very different. DeepPrime directly inputed the one-hot encoded sequence into the conventional network, while PRIDICT used an embedding layer to convert the one-hot encoded sequence into a dense representation. The dense representation is considered very beneficial in the NLP task as it siginificantly reduces the dimensionality of the input data, and can capture the semantic meaning of the the relationship between the words\cite{goldbergPrimerNeuralNetwork2015}. This can be very helpful in other bioinformatic tasks where a segment of genome sequence is considered as a single word, resulting a large vocabulary\cite{cegliaIdentificationTranscriptionalPrograms2023}. However, since we are working on the nucleotide level of representation, the vocabulary size is very small (4), and the semantic meaning of the nucleotides are not as important as the actual sequence. 

% Additionally, how the output of the attention heads are combined may also influence the model's performance.
% The common method of combining the output of a number of attention heads is to stack them into a single vector along the attention dimension, thus reshaping the output tensor from \texttt{(batch, sequence length, number of heads, head dimension)} to \texttt{(batch, sequence length, number of heads * head dimension)}. The reshaped tensor is then passed through a linear layer to reduce the dimension to the original head dimension. At the same time, we can also concatenate or take the mean of the output of the attention heads, resulting in a tensor of shape \texttt{(batch, sequence length * number of heads, head dimension)} and \texttt{(batch, sequence length, head dimension)} respectively.

The two embedding method and three attention head combination methods were tested on the subset of the dataset, and the results were compared using the Pearson correlation coefficient. 

The last stage focuses on the parameter size of the model. 
...
To remove the added variance of feature selection, all models were reconfigured to use the top 24 features extracted from the SHAP analysis. 

For the same reason, the MLP model and embedding was also reconfigured to use the same architecture so that the only difference between the models is the sequence encoding model. 

Although both yu et al and mathis et al reported the usage of one-hot encoding in their publication, the actual implementations were very different. DeepPrime directly inputed the one-hot encoded sequence into the convolutional network, while PRIDICT used an embedding layer to convert the one-hot encoded sequence into a dense representation. The dense representation is considered very beneficial in the NLP task as it siginificantly reduces the dimensionality of the input data, and can capture the semantic meaning of the the relationship between the words\cite{goldbergPrimerNeuralNetwork2015}. This can be very helpful in other bioinformatic tasks where a segment of genome sequence is considered as a single word, resulting a large vocabulary\cite{cegliaIdentificationTranscriptionalPrograms2023}. However, since we are working on the nucleotide level of representation, the vocabulary size is very small (4), and the semantic meaning of the nucleotides are not as important as the actual sequence. 
