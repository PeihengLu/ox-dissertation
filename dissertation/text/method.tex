...
Taking into account the performance of the DeepPrime model, the log adjustment was chosen as the primary method to address the imbalance in the dataset. The log adjustment was applied to the target values of all datasets used in this study from this point on.
...
Most of the models tested in this study so far are already using ensembling learning. The XGBoost model is an ensemble of multiple decision trees using gradient boosting, while DeepPrime, PRIDICT and the Transformer model are effectively an ensemble of a deep learning model and a MLP.

Thus, in this section, the focus is on combining these models to form a higher level parallel ensemble, where the models are trained independently and in parallel.
