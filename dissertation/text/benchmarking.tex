...
\begin{figure}
    ...
    Four models in total were produced in this study - the transformer based model (tr), the power weighted mean ensemble (pwm), the bagging ensemble (bg), and the adaboost ensemble (ab). Their performance is highlighted in the heatmap with the red bounding box.
    They were compared with the two state of the art deep learning solutions, DeepPrime (dp) and PRIDICT (pd). The datasets where any of the models trained on this study significantly outperformed both DeepPrime and PRIDICT are marked with a blue bounding box (performance across five folds, $p<0.05$, paired t-test).
    }
